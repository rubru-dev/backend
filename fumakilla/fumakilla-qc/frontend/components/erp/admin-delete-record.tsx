...
export function AdminDeleteRecord() {
  ...
  const remove = async () => { const warning = resource === "customers" ? "Hapus customer ini? Inquiry, quotation, survey, file, dan data terkait customer ini juga akan terhapus." : "Hapus inquiry ini? Tindakan ini tidak dapat dibatalkan."; if (!window.confirm(warning)) return; setDeleting(true); setError(""); try { await api.delete(`/erp/${resource}/${id}`); router.push(`/${resource}`); } catch (requestError: any) { setError(requestError.response?.data?.error || `${label} gagal dihapus.`); setDeleting(false); } };
