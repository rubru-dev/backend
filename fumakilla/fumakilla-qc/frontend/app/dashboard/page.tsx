...
function StatCard({ label, value, sub, subLabel, color, bg }: {
  ...
  return (
    <div style={{ background: bg, border: `1px solid ${color}30`, borderRadius: 12, padding: "16px 20px", minWidth: 0 }}>
      <p style={{ fontSize: 10, fontWeight: 800, color: "#6b7280", textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 8 }}>{label}</p>
      <p style={{ fontSize: 26, fontWeight: 900, color, lineHeight: 1.1 }}>{value}</p>
      {sub != null && (
...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="p-6 md:p-8">
      <PageTitle
        title="Dashboard Redaksi"
        subtitle={`Ringkasan data operasional Fumakilla · ${new Date().toLocaleDateString("id-ID", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}`}
        actions={null}
      />
      ...
      {loading ? <div className="mt-8"><Loading /></div> : !s ? (
        ...
        <>
          {/* ── Stat Cards Row 1 ── */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, margin: "24px 0 10px" }}>
