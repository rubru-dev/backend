...
export default function DrawLayoutPage() {
  ...
  const DRAFT_KEY = `dl_draft_${id}`;
  const [layoutName, setLayoutName] = useState("");
  const [loadingLayout, setLoadingLayout] = useState(true);
  const [dirty, setDirty] = useState(false);
  const [saving, setSaving] = useState(false);
  const [lastSavedAt, setLastSavedAt] = useState<number | null>(null);
  const loadedRef = useRef(false);           // true setelah data awal termuat (agar autosave tak jalan saat load)
  ...
  // Terapkan snapshot (dari server atau draft lokal) ke kanvas
  const applySnapshot = useCallback((d: any) => {
    pushHist(Array.isArray(d?.shapes) ? d.shapes : []);
    if (typeof d?.gridSize === "number") setGridSize(d.gridSize);
    if (typeof d?.bgOpacity === "number") setBgOpacity(d.bgOpacity);
    if (typeof d?.bgSrc === "string" && d.bgSrc) {
      setBgSrc(d.bgSrc);
      const i = new window.Image();
      i.onload = () => setBgImg(i);
      i.src = d.bgSrc;
  ...
  // Simpan ke backend (manual & autosave)
  const saveNow = useCallback(async () => {
    setSaving(true);
    try {
      await api.put(`/draw-layouts/${id}`, { name: layoutName, data: { shapes, gridSize, bgSrc, bgOpacity } });
      setDirty(false); dirtyRef.current = false;
      setLastSavedAt(Date.now());
      try { localStorage.removeItem(DRAFT_KEY); } catch {}
    } catch (e: any) {
      showAlert({ title: "Gagal menyimpan", message: e?.response?.data?.error || "Layout gagal disimpan. Cek koneksi.", tone: "danger" });
    } finally {
      setSaving(false);
    }
  }, [id, layoutName, shapes, gridSize, bgSrc, bgOpacity, DRAFT_KEY]);
  ...
  const handleBack = async () => {
    if (dirtyRef.current) {
      const ok = await showConfirm({
        ...
        message: "Simpan layout sebelum kembali ke daftar?",
    ...
    router.push("/draw-layout");
  };

  // ── Muat layout awal (+ pulihkan draft bila mati mendadak) ──
  useEffect(() => {
    let cancelled = false;
    (async () => {
      ...
      try {
        const res = await api.get(`/draw-layouts/${id}`);
        if (cancelled) return;
        ...
        if (draft?.savedAt && draft.savedAt > serverTime + 1500) {
          const ok = await showConfirm({
            title: "Pulihkan perubahan?",
            message: "Ada perubahan yang belum sempat tersimpan dari sesi sebelumnya. Pulihkan sekarang?",
          ...
          if (ok) {
            applySnapshot(draft);
            if (typeof draft.layoutName === "string") setLayoutName(draft.layoutName);
        ...
        if (!cancelled) showAlert({ title: "Gagal memuat", message: e?.response?.data?.error || "Layout tidak ditemukan.", tone: "danger" });
      } finally {
        if (!cancelled) {
  ...
  }, [id, applySnapshot, DRAFT_KEY]);

  // ── Autosave: draft lokal (anti mati mendadak) + backend (cross-device) ──
  useEffect(() => {
    ...
    draftTimer.current = setTimeout(() => {
      try { localStorage.setItem(DRAFT_KEY, JSON.stringify({ shapes, gridSize, bgSrc, bgOpacity, layoutName, savedAt: Date.now() })); } catch {}
    }, 800);

    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => { saveNowRef.current(); }, 4000);
  }, [shapes, gridSize, bgSrc, bgOpacity, layoutName, DRAFT_KEY]);

  // ── Peringatan tutup/refresh tab bila ada perubahan belum tersimpan ──
  useEffect(() => {
  ...
  // ── Peringatan pindah menu (klik link internal) bila belum tersimpan ──
  useEffect(() => {
    const onClickCapture = (e: MouseEvent) => {
      ...
      if (href.startsWith(`/draw-layout/${id}`)) return;
      e.preventDefault();
      e.stopPropagation();
      showConfirm({
        title: "Perubahan belum disimpan",
        message: "Simpan layout sebelum meninggalkan halaman ini?",
        confirmLabel: "Simpan & keluar",
        cancelLabel: "Batal",
      }).then(async (ok) => {
        if (!ok) return;
  ...
  const importBg = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = ev => {
      ...
      const i = new window.Image();
      i.onload = () => { setBgImg(i); setBgSrc(src); };
      i.src = src;
    };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 80px)", overflow: "hidden", userSelect: "none" }}>
      ...
      {/* ── Header (nama layout + simpan) ─────────────────────────────────── */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 12px", background: "#1e293b", borderBottom: "1px solid #0b1220", flexShrink: 0 }}>
        <button onClick={handleBack} title="Kembali ke daftar layout"
          style={{ height: 30, padding: "0 10px", borderRadius: 5, background: "rgba(255,255,255,0.08)", color: "#e2e8f0", border: "none", cursor: "pointer", fontSize: 12, flexShrink: 0 }}>
          ← Daftar
        </button>
        <input value={layoutName} onChange={e => setLayoutName(e.target.value)} placeholder="Nama layout…"
          style={{ height: 30, minWidth: 200, maxWidth: 360, flex: "0 1 auto", borderRadius: 5, border: "1px solid #334155", background: "#0f172a", color: "#f1f5f9", fontSize: 13, fontWeight: 600, padding: "0 10px" }} />
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          <span style={{ fontSize: 11, fontWeight: 600, color: saving ? "#93c5fd" : dirty ? "#fbbf24" : "#4ade80" }}>
            {loadingLayout ? "Memuat…" : saving ? "Menyimpan…" : dirty ? "● Belum tersimpan" : lastSavedAt ? `✓ Tersimpan ${new Date(lastSavedAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}` : "✓ Tersimpan"}
          </span>
          <button onClick={() => saveNow()} disabled={saving || loadingLayout} title="Simpan layout ke server (Ctrl+S)"
            style={{ height: 30, padding: "0 14px", borderRadius: 5, background: saving ? "#1d4ed8" : "#2563eb", color: "#fff", border: "none", cursor: saving ? "default" : "pointer", fontSize: 12, fontWeight: 700, opacity: saving || loadingLayout ? 0.6 : 1 }}>
            💾 Simpan
          </button>
        </div>
      </div>
      ...
      <div style={{ minHeight: TOOLBAR_H, background: "#0f172a", display: "flex", flexWrap: "wrap", alignItems: "center", gap: 3, rowGap: 6, padding: "6px 8px", flexShrink: 0 }}>
        ...
        {bgImg && (
          <>
            ...
            <Btn active={false} onClick={() => { setBgImg(null); setBgSrc(null); }} title="Hapus latar"><span style={{ fontSize: 10, color: "#f87171" }}>✕ Latar</span></Btn>
          </>
        )}
