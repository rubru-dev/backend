...
const STATUS_COLOR: Record<string, string> = {
  BERJALAN: "bg-[#0B7B7B] text-white",
  "Dalam Proses": "bg-[#0B7B7B] text-white",
...
export default async function GalleryPage({ searchParams }: PageProps) {
  ...
  try {
    data = await publicApi.rb.projects({ per_page, page });
  } catch {
    data = null;
  }
  const projects: RbProjectListItem[] = (data?.items ?? []) as RbProjectListItem[];
  const total: number = data?.total ?? 0;
  const totalPages = data?.total_pages ?? Math.ceil(total / per_page);

  const buildUrl = (pg: number) => {
    if (pg <= 1) return "/projek-berjalan";
    return `/projek-berjalan?page=${pg}`;
  };
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        {/* Grid */}
        {projects.length === 0 ? (
          <div className="text-center py-24 text-slate-400">Belum ada project tersedia.</div>
        ) : (
              ...
              return (
                <div key={p.id} className={`card flex flex-col${idx >= 4 ? " hidden sm:flex" : ""}`}>
                  ...
                  <div className="relative h-48 bg-slate-100 overflow-hidden flex-shrink-0">
                    ...
                    <span className={`absolute top-3 right-3 text-xs px-2.5 py-1 rounded-full font-semibold ${STATUS_COLOR[p.status] ?? "bg-slate-500 text-white"}`}>
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <p className="text-sm font-semibold text-[#FF9122] mb-1">{formatRupiah(p.budget)}</p>
                    {lokasi && (
                    ...
                    <div className="mb-3">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-xs text-slate-500">Progres Pengerjaan</span>
                        <span className="text-xs font-semibold text-[#FF9122]">{p.progress}%</span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-2">
                        <div
                          className="bg-[#FF9122] h-2 rounded-full transition-all"
