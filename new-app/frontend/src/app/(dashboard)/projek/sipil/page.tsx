...
export default function ProyekSipilListPage() {
  ...
  const { data: leads = [] } = useQuery<{ id: string; nama: string }[]>({
    ...
    queryFn: () => sipilApi.listLeads(deferredLeadSearch),
    enabled: dialog,
    staleTime: 5 * 60_000,
    retry: false,
  });
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-2 font-medium min-w-0">
                <Building2 className="h-4 w-4 text-teal-500 flex-shrink-0 mt-0.5" />
      ...
      <div className="hidden sm:block overflow-x-auto">
        <Table>
          ...
          <TableBody>
            ...
            {!isLoading && projeks.map((p) => (
              <TableRow key={p.id} className="cursor-pointer hover:bg-muted/40 transition-colors" onClick={() => router.push(`/projek/sipil/${p.id}`)}>
                <TableCell>
                  <div className="flex items-center gap-2 font-medium">
                    <Building2 className="h-4 w-4 text-teal-500 flex-shrink-0" />{p.nama_proyek ?? "—"}
