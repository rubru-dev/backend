...
function CardDetailModal({
  ...
  card: AdminKanbanCard;
  open: boolean;
  onClose: () => void;
  onSave: (id: number, data: Partial<AdminKanbanCard & { tanggal_survey?: string | null }>, leadTanggalMasuk?: string | null) => Promise<void>;
...
function KanbanCardComp({
  ...
  card: AdminKanbanCard;
  index: number;
  onEdit: (card: AdminKanbanCard) => void;
}) {
...
function KanbanColumnComp({
  ...
  column: AdminKanbanColumn;
  leads: { id: number; nama: string }[];
  bulan: number;
  tahun: number;
  onCardEdit: (card: AdminKanbanCard) => void;
...
export function AdminKanbanBoard() {
  ...
  const [columns, setColumns] = useState<AdminKanbanColumn[]>([]);
  ...
  const loadBoard = useCallback(async () => {
    ...
    try {
      const [cols, ls] = await Promise.all([
        adminKanbanApi.getBoard(bulan, tahun),
        adminKanbanApi.getLeads(),
      ]);
      setColumns(cols);
      setLeads(ls);
    } catch {
  ...
  const handleDragEnd = useCallback(async (result: DropResult) => {
    ...
    setColumns((prev) => {
      ...
      // drag = copy: keep source, also add to dest
    ...
    try {
      await adminKanbanApi.moveCard(cardId, targetColumnId, bulan, tahun);
      await loadBoard(); // reload to get real IDs
    } catch {
  ...
  const handleCardCreate = useCallback(async (columnId: number, data: { title: string; lead_id?: number | null }) => {
    try {
      await adminKanbanApi.createCard(columnId, data);
      await loadBoard();
  ...
  const handleCardSave = useCallback(async (id: number, data: Partial<AdminKanbanCard & { tanggal_survey?: string | null }>, leadTanggalMasuk?: string | null) => {
    await adminKanbanApi.updateCard(id, data as Parameters<typeof adminKanbanApi.updateCard>[1]);
    if (leadTanggalMasuk !== undefined) {
      const card = columns.flatMap((c) => c.cards).find((c) => c.id === id);
      if (card?.lead?.id) {
        await adminKanbanApi.updateLead(card.lead.id, { tanggal_masuk: leadTanggalMasuk });
      }
    }
    await loadBoard();
  }, [loadBoard, columns]);
  ...
  const handleCardDelete = useCallback(async (id: number) => {
    try {
      await adminKanbanApi.deleteCard(id);
      await loadBoard();
  ...
  const handleColumnUpdate = useCallback(async (id: number, data: { title?: string; color?: string }) => {
    try {
      await adminKanbanApi.updateColumn(id, data);
      await loadBoard();
    } catch {
  ...
  const handleColumnDelete = useCallback(async (id: number) => {
    ...
    try {
      await adminKanbanApi.deleteColumn(id);
      await loadBoard();
  ...
  const handleAddColumn = useCallback(async () => {
    ...
    try {
      await adminKanbanApi.createColumn({ title: newColumnTitle.trim(), bulan, tahun });
      setNewColumnTitle("");
  ...
  async function handleDownloadPdf() {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Kanban Admin — ${periodeLabel}</title><style>
      * { margin:0; padding:0; box-sizing:border-box; }
      ...
      <div class="meta">
        <div class="meta-item">
          ...
          <div class="value">Rekap Kanban Admin</div>
        </div>
        <div class="meta-item">
  ...
  const handleCarryover = useCallback(async () => {
    ...
    try {
      await adminKanbanApi.carryover({
