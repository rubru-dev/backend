...
function ComparisonChart({ title, ranges, data }: { title: string; ranges: any; data: Array<{ name: string; current: number; previous: number }> }) {
  return <div className="rounded-lg border-2 bg-card p-3 shadow-sm sm:p-4"><div className="mb-3"><h3 className="text-sm font-bold">{title}</h3><p className="text-xs text-muted-foreground"><span className="font-semibold text-blue-700">A: {ranges?.current?.label}</span> dibanding <span className="font-semibold text-orange-600">B: {ranges?.previous?.label}</span></p></div><div className="h-[280px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ top: 28, right: 8, left: -18, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" tick={{ fontSize: 11, fontWeight: 600 }} /><YAxis tick={{ fontSize: 10 }} tickFormatter={(value) => Number(value).toLocaleString("id-ID", { notation: "compact" })} /><Tooltip formatter={(value) => number(value)} /><Legend wrapperStyle={{ fontSize: 11, fontWeight: 600 }} /><Bar dataKey="current" name="Periode A" fill="#1d4ed8" radius={[5, 5, 0, 0]}><LabelList dataKey="current" position="top" formatter={(value: unknown) => Number(value).toLocaleString("id-ID", { notation: "compact" })} className="fill-blue-800 text-[10px] font-bold" /></Bar><Bar dataKey="previous" name="Periode B" fill="#f97316" radius={[5, 5, 0, 0]}><LabelList dataKey="previous" position="top" formatter={(value: unknown) => Number(value).toLocaleString("id-ID", { notation: "compact" })} className="fill-orange-700 text-[10px] font-bold" /></Bar></BarChart></ResponsiveContainer></div></div>;
}

function MultiMetricComparison({ title, metrics, selected, onChange, current, previous, ranges }: { title: string; metrics: Array<{ value: string; label: string; money?: boolean }>; selected: string[]; onChange: (values: string[]) => void; current: Record<string, number>; previous: Record<string, number>; ranges: any }) {
  const toggle = (metric: string) => {
    if (selected.includes(metric)) {
      if (selected.length > 1) onChange(selected.filter((item) => item !== metric));
    } else if (selected.length < 6) onChange([...selected, metric]);
  };
  const valueFormat = (metric: { value: string; money?: boolean }, value: unknown) => metric.money ? rupiah(value) : metric.value.includes("rate") || metric.value === "ctr" ? percentage(value) : number(value);
  const shortFormat = (metric: { value: string; money?: boolean }, value: unknown) => metric.money
    ? new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", notation: "compact", maximumFractionDigits: 1 }).format(Number(value ?? 0))
    : metric.value.includes("rate") || metric.value === "ctr" ? percentage(value) : Number(value ?? 0).toLocaleString("id-ID", { notation: "compact", maximumFractionDigits: 1 });
  return <section className="space-y-4 rounded-lg border-2 bg-card p-3 shadow-sm sm:p-4">
    <div><h3 className="text-sm font-bold">{title}</h3><p className="text-xs text-muted-foreground"><span className="font-semibold text-blue-700">A: {ranges?.current?.label}</span> dibanding <span className="font-semibold text-orange-600">B: {ranges?.previous?.label}</span></p></div>
    <div className="flex flex-wrap gap-2">{metrics.map((metric) => { const active = selected.includes(metric.value); return <button type="button" key={metric.value} aria-pressed={active} disabled={!active && selected.length >= 6} onClick={() => toggle(metric.value)} className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-colors ${active ? "border-blue-700 bg-blue-700 text-white" : "bg-background text-muted-foreground hover:border-blue-400 disabled:cursor-not-allowed disabled:opacity-40"}`}>{metric.label}</button>; })}</div>
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{selected.map((metricKey) => { const metric = metrics.find((item) => item.value === metricKey)!; const a = Number(current?.[metricKey] ?? 0); const b = Number(previous?.[metricKey] ?? 0); const delta = b !== 0 ? ((a - b) / Math.abs(b)) * 100 : null; return <article key={metricKey} className="rounded-lg border bg-background p-3"><div className="flex items-start justify-between gap-2"><h4 className="text-sm font-bold">{metric.label}</h4>{delta != null && <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${delta >= 0 ? "bg-blue-100 text-blue-800" : "bg-orange-100 text-orange-800"}`}>{delta >= 0 ? "+" : ""}{delta.toFixed(1)}%</span>}</div><div className="mt-2 h-[210px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={[{ name: metric.label, current: a, previous: b }]} margin={{ top: 30, right: 5, left: 5, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><YAxis hide domain={[0, "auto"]} /><Tooltip formatter={(value) => valueFormat(metric, value)} /><Legend wrapperStyle={{ fontSize: 10, fontWeight: 600 }} /><Bar dataKey="current" name="Periode A" fill="#1d4ed8" radius={[6, 6, 0, 0]} maxBarSize={70}><LabelList dataKey="current" position="top" formatter={(value: unknown) => shortFormat(metric, value)} className="fill-blue-800 text-[10px] font-bold" /></Bar><Bar dataKey="previous" name="Periode B" fill="#f97316" radius={[6, 6, 0, 0]} maxBarSize={70}><LabelList dataKey="previous" position="top" formatter={(value: unknown) => shortFormat(metric, value)} className="fill-orange-700 text-[10px] font-bold" /></Bar></BarChart></ResponsiveContainer></div></article>; })}</div>
    <p className="text-[11px] text-muted-foreground">Pilih sampai 6 metrik. Setiap metrik memakai skala sendiri agar Rupiah, jumlah, dan persentase tetap terbaca jelas.</p>
  </section>;
}
...
function ReportAnalyticsContent() {
  ...
  const [adsCompareMetrics, setAdsCompareMetrics] = useState(["spend", "clicks", "ctr"]);
  const [socialCompareMetrics, setSocialCompareMetrics] = useState(["views", "reach", "engagement_rate"]);
  const [closingCompareMetrics, setClosingCompareMetrics] = useState(["total", "nominal"]);
  ...
  return <div className="space-y-5">
    ...
    <Tabs value={activeTab} onValueChange={changeTab}>
      ...
      {comparePeriod.mode !== "custom" && Boolean(startDate) !== Boolean(endDate) && <div className="mt-3 rounded-lg border border-dashed p-4 text-sm text-muted-foreground">Isi tanggal mulai dan tanggal selesai untuk menampilkan grafik perbandingan otomatis, atau pilih dua periode kustom.</div>}
      {isLoading ? <p className="py-12 text-center text-sm text-muted-foreground">Memuat laporan...</p> : isError ? <p className="py-12 text-center text-sm text-destructive">{(error as any)?.response?.data?.detail || "Gagal memuat laporan BD."}</p> : <>
        <TabsContent value="ads" className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-muted-foreground">Sumber: {data?.ads_organik?.ads_data_source === "realtime" ? "Meta Ads API" : data?.ads_organik?.ads_data_source === "manual" ? "Data lokal/manual" : "Data lokal (fallback)"}</p><SearchBox value={search} onChange={(value) => { setSearch(value); setPage(1); }} placeholder="Cari campaign..." /></div>
          ...
          {comparisonLoading ? <div className="h-[290px] animate-pulse rounded-lg bg-muted" /> : comparison && <MultiMetricComparison title="Perbandingan Performa Ads" metrics={ADS_COMPARISON_METRICS} selected={adsCompareMetrics} onChange={setAdsCompareMetrics} current={comparison.current.ads} previous={comparison.previous.ads} ranges={comparison.ranges} />}
        ...
        <TabsContent value="social" className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="inline-grid grid-cols-3 rounded-lg border bg-muted/30 p-1">{["Instagram", "YouTube", "TikTok"].map((platform) => <button key={platform} className={`rounded px-3 py-2 text-xs ${socialTab === platform ? "bg-background font-medium shadow-sm" : "text-muted-foreground"}`} onClick={() => setSocialTab(platform)}>{platform}</button>)}</div><SearchBox value={search} onChange={(value) => { setSearch(value); setPage(1); }} placeholder="Cari akun atau konten..." /></div>
          ...
          {comparisonLoading ? <div className="h-[290px] animate-pulse rounded-lg bg-muted" /> : comparison && <MultiMetricComparison title={`Perbandingan ${socialTab}`} metrics={SOCIAL_COMPARISON_METRICS} selected={socialCompareMetrics} onChange={setSocialCompareMetrics} current={comparison.current.social[socialTab.toLowerCase()]} previous={comparison.previous.social[socialTab.toLowerCase()]} ranges={comparison.ranges} />}
        ...
        <TabsContent value="funnel" className="space-y-4"><p className="text-xs text-muted-foreground">Data mengikuti Kalender Survey: semua modul yang tampil di kalender, dengan jadwal pada periode berdasarkan tanggal survey.</p>
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Metric label="Lead Terjadwal Survey" value={number(funnel.total_leads)} /><Metric label="Survey" value={number(funnel.survey)} detail="Sesuai Kalender Survey" /><Metric label="DP Desain Lunas" value={number(funnel.dp_desain)} detail={`${percentage(funnel.conversion_survey_to_design)} konversi · ${number(Math.max(0, (funnel.survey || 0) - (funnel.dp_desain || 0)))} drop-off`} /><Metric label="SPK Projek" value={number(funnel.spk)} detail={`${percentage(funnel.conversion_design_to_spk)} konversi · ${number(Math.max(0, (funnel.dp_desain || 0) - (funnel.spk || 0)))} drop-off`} /></div>
          {Array.isArray(funnel.survey_by_source) && funnel.survey_by_source.length > 0 && <section className="space-y-2"><h2 className="text-sm font-semibold">Survey per Sumber</h2><ResponsiveTable headers={["Sumber", "Jumlah Survey"]} rows={funnel.survey_by_source} minWidth="520px" renderRow={(row) => <tr key={row.sumber} className="border-t"><td className="px-3 py-2 font-medium">{row.sumber}</td><td className="px-3 py-2">{number(row.total)}</td></tr>} renderMobile={(row) => <DetailCard id={row.sumber} title={row.sumber} fields={[["Jumlah Survey", number(row.total)]]} />} /></section>}
          ...
          {comparisonLoading ? <div className="h-[290px] animate-pulse rounded-lg bg-muted" /> : comparison && <ComparisonChart title="Perbandingan Funnel" ranges={comparison.ranges} data={[{ name: "Leads", current: comparison.current.funnel.leads, previous: comparison.previous.funnel.leads }, { name: "Survey", current: comparison.current.funnel.survey, previous: comparison.previous.funnel.survey }, { name: "DP Desain", current: comparison.current.funnel.design, previous: comparison.previous.funnel.design }, { name: "SPK", current: comparison.current.funnel.spk, previous: comparison.previous.funnel.spk }]} />}
          <SearchBox value={search} onChange={(value) => { setSearch(value); setPage(1); }} placeholder="Cari lead atau sales admin..." />
          <ResponsiveTable headers={["Lead", "Sumber", "Modul", "Inputter", "Tgl. Masuk", "Survey", "DP Desain", "SPK", "Survey → DP", "DP → SPK", "DP Projek Lunas"]} rows={paginate(filteredFunnel)} minWidth="1400px" renderRow={(row) => <tr key={row.lead_id} className="border-t"><td className="px-3 py-2 font-medium">{row.nama}</td><td className="px-3 py-2">{row.sumber}</td><td className="px-3 py-2">{row.modul}</td><td className="px-3 py-2">{row.sales}</td><td className="px-3 py-2">{date(row.tanggal_masuk)}</td><td className="px-3 py-2">{date(row.tanggal_survey)}</td><td className="px-3 py-2">{date(row.tanggal_dp_desain)}</td><td className="px-3 py-2">{date(row.tanggal_spk)}</td><td className="px-3 py-2">{days(row.days_survey_to_design_dp)}</td><td className="px-3 py-2">{days(row.days_design_to_spk)}</td><td className="px-3 py-2">{rupiah(row.dp_projek_lunas)}</td></tr>} renderMobile={(row) => <DetailCard id={row.lead_id} title={row.nama} fields={[["Sumber", row.sumber], ["Modul", row.modul], ["Inputter", row.sales], ["Tanggal Masuk", date(row.tanggal_masuk)], ["Survey", date(row.tanggal_survey)], ["DP Desain", date(row.tanggal_dp_desain)], ["SPK", date(row.tanggal_spk)], ["Survey → DP", days(row.days_survey_to_design_dp)], ["DP → SPK", days(row.days_design_to_spk)]]} />} /><Pager page={page} total={filteredFunnel.length} onChange={setPage} />
        </TabsContent>
        ...
        <TabsContent value="closing" className="space-y-4"><p className="text-xs text-muted-foreground">Closing dihitung saat kartu masuk ke kolom Closing. Data lama yang belum memiliki tanggal perpindahan memakai tanggal pembuatan kartu dan diberi penanda.</p>
          ...
          {comparisonLoading ? <div className="h-[290px] animate-pulse rounded-lg bg-muted" /> : comparison && <MultiMetricComparison title="Perbandingan Closing" metrics={CLOSING_COMPARISON_METRICS} selected={closingCompareMetrics} onChange={setClosingCompareMetrics} current={comparison.current.closing} previous={comparison.previous.closing} ranges={comparison.ranges} />}
