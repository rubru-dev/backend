...
export function KanbanMetrics({ title = "Metrics Kanban BD" }: Props) {
  const now = new Date();
  const [month, setMonth] = useState(now.getMonth() + 1);
  const [year, setYear] = useState(now.getFullYear());

  const { data, isLoading } = useQuery({
    queryKey: ["bd-kanban-metrics", year, month],
    queryFn: () => bdKanbanApi.getMetrics(year, month),
  });
            ...
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={data?.timeline ?? []} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                  <Tooltip />
                  <Line type="monotone" dataKey="count" name="Dibuat" stroke="#6366f1" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            )}
          </CardContent>
