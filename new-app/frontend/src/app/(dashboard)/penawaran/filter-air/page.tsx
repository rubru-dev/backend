...
export default function PenawaranFilterAirPage() {
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      await downloadOfferPdf(".offer-page", `Penawaran Filter Air - ${name} - ${formatDateFile(tanggal)}`);
    } finally {
  ...
  function loadOffer(offer: SavedOffer, shouldPrint = false) {
    ...
    if (shouldPrint) {
      ...
      setTimeout(async () => {
        try {
          await downloadOfferPdf(".offer-page", `Penawaran Filter Air - ${offer.salutation}. ${offer.clientName} - ${formatDateFile(offer.tanggal)}`);
        } finally {
          setDownloadingPdf(false);
        }
      }, 100);
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 print:hidden">
        <div>
          <h1 className="text-2xl font-bold">Penawaran Filter Air</h1>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="print:hidden">
        ...
        <TabsContent value="generate" className="mt-4">
          <div className="grid gap-4 rounded-lg border bg-white p-4">
            <div className="grid md:grid-cols-4 gap-3">
              ...
              <div>
                <Label>Nama Client Filter Air</Label>
                <div className="relative">
                  <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                  <Input value={clientSearch} onChange={(e) => setClientSearch(e.target.value)} placeholder="Cari dari Follow Up Leads Filter Air" className="pl-9" />
                </div>
                <Select
        ...
        <TabsContent value="list" className="mt-4">
          <div className="rounded-lg border bg-white">
            <div className="grid grid-cols-[1.2fr_1fr_1fr_150px] gap-3 border-b px-4 py-3 text-xs font-semibold uppercase text-muted-foreground">
              <span>Client</span><span>Tanggal</span><span>Total</span><span className="text-right">Aksi</span>
            </div>
            {savedOffers.length ? savedOffers.map((offer) => (
              <div key={offer.id} className="grid grid-cols-[1.2fr_1fr_1fr_150px] items-center gap-3 border-b px-4 py-3 text-sm last:border-b-0">
      ...
      {showPreview && (
        <div className="offer-page mx-auto max-w-[794px] min-h-[1123px] border bg-white px-[1.5cm] py-[0.5cm] shadow-sm text-[12px] leading-5 text-black">
          <Letterhead />
          <h2 className="mb-8 text-center text-[14px] font-bold">Penawaran Filter Air by Rubah Rumah</h2>
          ...
          <p className="text-justify">
            Bersama surat ini kami PT. RUBAH RUMAH INOVASI PEMUDA mengajukan penawaran produk filter air kepada {name} dengan rincian sebagai berikut :
          </p>
          ...
          <table className="my-5 w-full border-collapse text-[12px]">
            <thead>
              <tr>
                ...
                <th className="border border-black p-2 text-left">Uraian Produk</th>
          ...
          <div className="-mx-[1.5cm] mt-8 ml-auto w-[300px] text-left">
            ...
            <div className="mt-3 flex h-20 w-40 items-center">
              <img src={LOGO_SRC} alt="Rubah Rumah" className="max-h-20 max-w-40 object-contain" />
            </div>
            <div className="h-8" />
    ...
    <div className="offer-header mb-6 border-b-2 border-black pb-4 text-[12px]">
      <div className="flex items-center gap-4">
        <div className="flex h-36 w-44 shrink-0 items-center justify-center">
          <img src={LOGO_SRC} alt="Rubah Rumah" className="max-h-36 max-w-44 object-contain" />
        </div>
        <div>
