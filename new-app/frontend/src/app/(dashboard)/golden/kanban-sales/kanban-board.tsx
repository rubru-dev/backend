...
// ── Main Board ─────────────────────────────────────────────────────────────────
export interface GoldenSalesKanbanBoardProps {
...
export function GoldenSalesKanbanBoard({ initialColumns, isLoading = false, onRefresh }: GoldenSalesKanbanBoardProps) {
  ...
  const { data: leadsData } = useQuery({
    queryKey: ["/golden-kanban-sales/kanban/leads"],
    queryFn: () => goldenKanbanSalesApi.getLeads(),
    staleTime: 60_000,
  });
  ...
  const handleCarryover = useCallback(async () => {
    ...
    try {
      const { copied } = await goldenKanbanSalesApi.carryover({ month: targetMonth, year: targetYear });
      onRefresh();
  ...
  const onDragEnd = useCallback(async (result: DropResult) => {
    ...
    if (type === "COLUMN") {
      ...
      try {
        await goldenKanbanSalesApi.reorderColumns(reordered.map((c) => Number(c.id)));
      } catch {
    ...
    try {
      const res = await goldenKanbanSalesApi.createCard({
        title: srcCard.title,
  ...
  async function handleAddColumn(title: string) {
    ...
    try {
      const res = await goldenKanbanSalesApi.createColumn({ title, urutan, color: "#e2e8f0" });
  ...
  async function handleUpdateColumn(id: number, data: { title?: string; color?: string }) {
    ...
    try {
      await goldenKanbanSalesApi.updateColumn(id, data);
    } catch {
  ...
  async function handleDeleteColumn(id: number) {
    try {
      await goldenKanbanSalesApi.deleteColumn(id);
  ...
  async function handleAddCard(columnId: number, leadId: number, leadNama: string) {
    try {
      const res = await goldenKanbanSalesApi.createCard({ title: leadNama, lead_id: leadId, column_id: columnId });
  ...
  async function handleUpdateCard(id: number, data: Partial<KanbanCard>) {
    await goldenKanbanSalesApi.updateCard(id, {
      title: data.title,
  ...
  function handleColorChange(id: number, color: string | null) {
    ...
    goldenKanbanSalesApi.updateCard(id, { color: color ?? undefined }).catch(() => {
  ...
  async function handleDeleteCard(cardId: number) {
    try {
      await goldenKanbanSalesApi.deleteCard(cardId);
      setColumns((prev) =>
