  ...
  async function handleSave() {
    ...
    try {
      await onSave(card.id, {
        ...
        projeksi_sales: projeksiSales ? parseFloat(projeksiSales) : null,
...
function KanbanCardComp({
  ...
  card: GoldenKanbanAdminCard;
  index: number;
  onEdit: (card: GoldenKanbanAdminCard) => void;
}) {
...
function KanbanColumnComp({
  ...
  bulan: number;
  tahun: number;
  onCardEdit: (card: GoldenKanbanAdminCard) => void;
      ...
      >
        {editingTitle && !isPermanent ? (
          <div className="flex gap-1 flex-1">
            ...
            <button
              onClick={async () => {
                await onColumnUpdate(column.id, { title: colTitle });
        ...
        <div className="bg-white border rounded p-2 grid grid-cols-4 gap-1 shadow">
          {COLUMN_COLORS.map((c) => (
            <button
              ...
              onClick={async () => {
                await onColumnUpdate(column.id, { color: c.hex });
...
export function GoldenAdminKanbanBoard() {
  ...
  const [columns, setColumns] = useState<GoldenKanbanAdminColumn[]>([]);
  const [loading, setLoading] = useState(true);
  const [leads, setLeads] = useState<{ id: number; nama: string }[]>([]);
  const [selectedCard, setSelectedCard] = useState<GoldenKanbanAdminCard | null>(null);
  ...
  const loadBoard = useCallback(async () => {
    ...
    try {
      const [cols, ls] = await Promise.all([
        goldenKanbanAdminApi.getBoard(bulan, tahun),
        goldenKanbanAdminApi.getLeads(),
      ]);
      setColumns(cols);
      setLeads(ls);
    } catch {
  ...
  const handleDragEnd = useCallback(async (result: DropResult) => {
    ...
    try {
      await goldenKanbanAdminApi.moveCard(cardId, targetColumnId, bulan, tahun);
      await loadBoard();
    } catch {
  ...
  const handleCardCreate = useCallback(async (columnId: number, data: { title: string; lead_id?: number | null }) => {
    try {
      await goldenKanbanAdminApi.createCard(columnId, data);
      await loadBoard();
  ...
  const handleCardSave = useCallback(async (id: number, data: Partial<GoldenKanbanAdminCard & { tanggal_survey?: string | null }>, leadTanggalMasuk?: string | null) => {
    await goldenKanbanAdminApi.updateCard(id, data as Parameters<typeof goldenKanbanAdminApi.updateCard>[1]);
    if (leadTanggalMasuk !== undefined) {
      const card = columns.flatMap((c) => c.cards).find((c) => c.id === id);
      if (card?.lead?.id) {
        await goldenKanbanAdminApi.updateLead(card.lead.id, { tanggal_masuk: leadTanggalMasuk });
      }
    }
    await loadBoard();
  }, [loadBoard, columns]);
  ...
  const handleCardDelete = useCallback(async (id: number) => {
    try {
      await goldenKanbanAdminApi.deleteCard(id);
      await loadBoard();
  ...
  const handleColumnUpdate = useCallback(async (id: number, data: { title?: string; color?: string }) => {
    try {
      await goldenKanbanAdminApi.updateColumn(id, data);
      await loadBoard();
    } catch {
  ...
  const handleColumnDelete = useCallback(async (id: number) => {
    ...
    try {
      await goldenKanbanAdminApi.deleteColumn(id);
      await loadBoard();
  ...
  const handleAddColumn = useCallback(async () => {
    ...
    try {
      await goldenKanbanAdminApi.createColumn({ title: newColumnTitle.trim(), bulan, tahun });
      setNewColumnTitle("");
  ...
  async function handleDownloadPdf() {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Kanban Admin Golden — ${periodeLabel}</title><style>
      * { margin:0; padding:0; box-sizing:border-box; }
      ...
      <div class="meta">
        <div class="meta-item"><div class="label">Dokumen</div><div class="value">Rekap Kanban Admin Golden</div></div>
        <div class="meta-item"><div class="label">Periode</div><div class="value">${periodeLabel}</div></div>
        <div class="meta-item"><div class="label">Total Kolom</div><div class="value">${columns.length}</div></div>
        <div class="meta-item"><div class="label">Total Card</div><div class="value">${totalCards}</div></div>
      </div>
