...
router.get("/kanban", async (req: Request, res: Response) => {
  ...
  for (const col of PERMANENT_COLUMNS) {
    const exists = await prisma.goldenKanbanAdminColumn.findFirst({
      where: { title: col.title, bulan, tahun },
    });
    if (!exists) {
      await prisma.goldenKanbanAdminColumn.create({
        data: { ...col, bulan, tahun },
      });
    }
  }

  const columns = await prisma.goldenKanbanAdminColumn.findMany({
    where: { bulan, tahun },
    orderBy: { urutan: "asc" },
    include: {
      cards: {
...
// GET /kanban/leads — leads dropdown (golden modul)
...
router.post("/kanban/columns", async (req: Request, res: Response) => {
  ...
  const col = await prisma.goldenKanbanAdminColumn.create({
...
router.patch("/kanban/columns/:id", async (req: Request, res: Response) => {
  ...
  const col = await prisma.goldenKanbanAdminColumn.update({ where: { id }, data: { title, color, urutan } });
  return res.json(col);
});
...
router.delete("/kanban/columns/:id", async (req: Request, res: Response) => {
  ...
  await prisma.goldenKanbanAdminColumn.delete({ where: { id } });
...
router.post("/kanban/columns/:id/cards", async (req: Request, res: Response) => {
  ...
  const card = await prisma.goldenKanbanAdminCard.create({
    data: {
      column_id, title,
...
router.patch("/kanban/cards/:id", async (req: Request, res: Response) => {
  const id = BigInt(req.params.id);
  const { title, description, assigned_user_id, deadline, tanggal_survey, color, column_id, urutan, projeksi_sales } = req.body;
...
router.post("/kanban/cards/:id/move", async (req: Request, res: Response) => {
  ...
  const source = await prisma.goldenKanbanAdminCard.findUnique({
    where: { id: sourceId },
    include: { labels: true },
  });
  if (!source) return res.status(404).json({ detail: "Card tidak ditemukan" });

  const targetCol = await prisma.goldenKanbanAdminColumn.findUnique({
    where: { id: BigInt(target_column_id) },
  });
  if (!targetCol) return res.status(404).json({ detail: "Kolom tujuan tidak ditemukan" });

  const newCard = await prisma.goldenKanbanAdminCard.create({
    data: {
      column_id: BigInt(target_column_id), title: source.title, description: source.description,
      lead_id: source.lead_id, assigned_user_id: source.assigned_user_id,
      deadline: source.deadline, tanggal_survey: source.tanggal_survey,
      color: source.color, urutan: source.urutan,
    },
  });
...
router.get("/kanban/cards/:id/comments", async (req: Request, res: Response) => {
  ...
  const comments = await prisma.goldenKanbanAdminComment.findMany({
    where: { card_id },
...
router.post("/kanban/cards/:id/comments", async (req: Request, res: Response) => {
  ...
  const c = await prisma.goldenKanbanAdminComment.create({
