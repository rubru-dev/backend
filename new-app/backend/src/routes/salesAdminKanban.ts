...
const PERMANENT_COLUMNS = [
  ...
  { title: "W2",                    color: "#8b5cf6", urutan: 1 },
...
router.get("/kanban", async (req: Request, res: Response) => {
  ...
  for (const col of PERMANENT_COLUMNS) {
    const exists = await prisma.adminKanbanColumn.findFirst({
      where: { title: col.title, bulan, tahun },
    });
    if (!exists) {
      await prisma.adminKanbanColumn.create({
        data: { ...col, bulan, tahun },
      });
    }
  }

  const columns = await prisma.adminKanbanColumn.findMany({
    where: { bulan, tahun },
    orderBy: { urutan: "asc" },
    include: {
      cards: {
...
// GET /kanban/leads — leads dropdown (sales-admin modul)
...
// POST /kanban/carryover — copy cards from prev month to new month
router.post("/kanban/carryover", async (req: Request, res: Response) => {
  const { from_bulan, from_tahun, to_bulan, to_tahun } = req.body;

  const fromCols = await prisma.adminKanbanColumn.findMany({
  ...
  for (const fromCol of fromCols) {
    ...
    let toCol = await prisma.adminKanbanColumn.findFirst({
      where: { title: fromCol.title, bulan: to_bulan, tahun: to_tahun },
    });
    if (!toCol) {
      toCol = await prisma.adminKanbanColumn.create({
        data: {
          title: fromCol.title,
          color: fromCol.color,
...
router.post("/kanban/columns", async (req: Request, res: Response) => {
  ...
  const col = await prisma.adminKanbanColumn.create({
...
router.patch("/kanban/columns/:id", async (req: Request, res: Response) => {
  ...
  const col = await prisma.adminKanbanColumn.update({
    where: { id },
...
router.delete("/kanban/columns/:id", async (req: Request, res: Response) => {
  const id = BigInt(req.params.id);
  const col = await prisma.adminKanbanColumn.findUnique({ where: { id } });
  ...
  await prisma.adminKanbanColumn.delete({ where: { id } });
...
router.post("/kanban/columns/:id/cards", async (req: Request, res: Response) => {
  ...
  const card = await prisma.adminKanbanCard.create({
    data: {
      column_id,
      title,
...
router.patch("/kanban/cards/:id", async (req: Request, res: Response) => {
  ...
  const card = await prisma.adminKanbanCard.update({ where: { id }, data });
...
router.delete("/kanban/cards/:id", async (req: Request, res: Response) => {
  const id = BigInt(req.params.id);
  await prisma.adminKanbanCard.delete({ where: { id } });
  return res.json({ message: "Card dihapus" });
});

// POST /kanban/cards/:id/move — drag-copy to another column
// If target column is "Move To Telemarketing", also copy to Telemarketing Kanban
router.post("/kanban/cards/:id/move", async (req: Request, res: Response) => {
  ...
  const source = await prisma.adminKanbanCard.findUnique({
    where: { id: sourceId },
    include: { labels: true },
  });
  if (!source) return res.status(404).json({ detail: "Card tidak ditemukan" });

  const targetCol = await prisma.adminKanbanColumn.findUnique({
  ...
  const newCard = await prisma.adminKanbanCard.create({
    data: {
  ...
  if (targetCol.title === "Move To Telemarketing") {
    ...
    if (!fromAdminCol) {
      fromAdminCol = await prisma.telemarketingKanbanColumn.create({
        data: {
          title: "From Sales Admin",
          color: "#6366f1",
          urutan: 0,
          bulan: tBulan,
          tahun: tTahun,
  ...
  // If moved to "Closing Survey" and card has lead, update lead tanggal_survey
...
router.patch("/kanban/cards/:id/survey", async (req: Request, res: Response) => {
  ...
  const card = await prisma.adminKanbanCard.update({
    where: { id },
...
router.get("/kanban/cards/:id/comments", async (req: Request, res: Response) => {
  ...
  const comments = await prisma.adminKanbanComment.findMany({
    where: { card_id },
...
router.post("/kanban/cards/:id/comments", async (req: Request, res: Response) => {
  ...
  const c = await prisma.adminKanbanComment.create({
